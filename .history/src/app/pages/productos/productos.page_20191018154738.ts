...
export class ProductosPage implements OnInit {
  ...
  async eliminar(producto, i) {
    try {
      // await this.productoService.borrarProducto(producto);
      // this.productos.splice(i, 1);
      this.productoService.borrarPrecioEnClientes(producto.id);
    } catch (error) {
      this.presentAlert('Error', error);
    }
  }
  ...
  async presentAlertConfirm(producto, i) {
    const alert = await this.alertController.create({
      header: 'Borrar producto',
      message: '¿Deseas borrar el producto de forma permanente?',
      buttons: [
        {
          text: 'Cancelar',
          role: 'cancel',
