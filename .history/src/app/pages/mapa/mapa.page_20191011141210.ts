...
export class MapaPage implements OnInit {
  ...
  async getBalance(dia) {
    this.balance = await this.repartidoreService.getBalance(dia);
    this.balance.forEach(repartidor => {
      const index = this.repartidores.findIndex(r => r.datos.id === repartidor.id);
      if (index >= 0) {
        repartidor.nombre = this.repartidores[index].datos.nombre;
        repartidor.foto = this.repartidores[index].datos.foto;
      }
    });
