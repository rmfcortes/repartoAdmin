...
export class ClientesPage implements OnInit, OnDestroy {
  ...
  listenClientesNuevos() {
    this.clienteService.nuevosClientes().query.ref.on('child_added', snapshot => {
      this.ngZone.run(() => {
        console.log('Nuevo cliente');
        const nuevoCliente = snapshot.val();
  ...
  ngOnDestroy() {
    this.clienteService.nuevosClientes().query.ref.off('child_added');
  }
