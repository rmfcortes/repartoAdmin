...
export class ClientesPage implements OnInit {
  ...
  async evaluaClientes(clientes: Cliente[], event?) {
    if (clientes.length === this.batch + 1) {
      const i = clientes.findIndex (c => c.cliente === this.lastKey);
      clientes.splice(i, 1);
      this.clientes = this.clientes.concat(clientes.reverse());
      this.clientesReady = true;
    } else if (this.clientes.length >= this.batch * this.infiniteCall) {
      this.clientes = this.clientes.concat(clientes.reverse());
      this.clientesReady = true;
    } else {
      this.clientes = this.clientes.concat(clientes.reverse());
  ...
  // acomodaClientes(clientes: Cliente[]): Promise<Cliente[]> {
  //   return new Promise(async (resolve, reject) => {
  //     for (const c of clientes) {
  //       c.dias = await this.diasTranscurridos(c.ultimaCompra, this.hoy);
  //     }
  //     clientes.sort((a, b) => b.acumulado - a.acumulado);
  //     resolve(clientes);
  //   });
  // }
