...
export class UsuariosService {
  ...
  borrarColaborador(colaborador: Usuario) {
    return new Promise(async (resolve, reject) => {
      try {
        if (colaborador.url) {
          await this.fireStorage.storage.refFromURL(colaborador.url).delete();
        }
        await this.db.object(`repartidores/${colaborador.id}`).remove();
        await this.db.object(`carga/${colaborador.id}`).remove();
        resolve();
      } catch (error) {
        reject(error);
      }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        if (!colaborador.id) {
          ...
          if (colaborador.puesto === 'Vendedor') {
            const datosCarga = {
              datos: {
                  id: colaborador.id,
                  nombre: colaborador.nombre,
                  telefono: colaborador.telefono,
                  foto: colaborador.url || null
              },
          };
