...
export class ClientesService {
  ...
  getClientesMayorCompra(batch, lastkey): Promise<Cliente[]> {
    return new Promise((resolve, reject) => {
      if (lastkey) {
        const clSub = this.db.list('clientes', data => data.orderByChild('acumulado').limitToLast(batch).endAt(null, lastkey))
        .valueChanges().subscribe((clientes: Cliente[]) => {
          clSub.unsubscribe();
          resolve(clientes);
        });
  ...
  getClientesMenorCompra(batch, lastkey): Promise<Cliente[]> {
    return new Promise((resolve, reject) => {
      if (lastkey) {
        const clSub = this.db.list('clientes', data => data.orderByChild('acumulado').limitToFirst(batch).startAt(lastkey))
        .valueChanges().subscribe((clientes: Cliente[]) => {
          clSub.unsubscribe();
          resolve(clientes);
        });
  ...
  getClientesCompraReciente(batch, lastkey): Promise<Cliente[]> {
    return new Promise((resolve, reject) => {
      if (lastkey) {
        const clSub = this.db.list('clientes', data => data.orderByChild('ultimaCompra').limitToLast(batch).startAt(lastkey))
        .valueChanges().subscribe((clientes: Cliente[]) => {
          clSub.unsubscribe();
          resolve(clientes);
        });
