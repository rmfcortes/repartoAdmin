...
export class PedidosService {
  ...
  async eliminarPedido(idPedido, idVendedor) {
    if (idVendedor) {
      await this.db.object(`pedidos/${idVendedor}/detalles/${idPedido}/eliminado`).set(true);
      await this.db.object(`pedidos/${idVendedor}/cantidad`).query.ref.transaction( count => count ? count - 1 : 0);
      await this.db.object(`pedidos/${idVendedor}/detalles/${idPedido}`).remove();
    } else {
      await this.db.object(`pedidos-pendientes/${idPedido}`).remove();
    }
  }
  ...
  async reAsignarPedido(pedido: Pedido, vendedor: Repartidores) {
    ...
    await this.db.object(`pedidos/${anteriorChofer}/detalles/${pedido.pedido.id}/reasignar`).set(true);
