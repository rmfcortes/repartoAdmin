...
export class RepartidoresService {
  ...
  getCompras(idVendedor, viaje, fecha) {
    return new Promise(async (resolve, reject) => {
      ...
      if (viaje && viaje >= 0) {
        comprasRef = this.db.list(`ventas/${fecha}/${idVendedor}/venta/${viaje}`);
      } else {
        comprasRef = this.db.list(`ventas/${fecha}/${idVendedor}/venta`);
      }
      const venSub = comprasRef.valueChanges().subscribe(ventas => {
        venSub.unsubscribe();
        if (ventas) {
          if (viaje && viaje >= 0) {
            resolve(ventas);
