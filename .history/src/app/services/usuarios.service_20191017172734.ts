    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (err.code === 'auth/email-already-in-use') {
          this.presentAlert('Correo registrado', 'Este correo ya ha sido registrado con otro usuario');
          reject('Este correo ya está registrado. Intenta con otro');
        } else if (err.code === 'auth/weak-password') {
          this.presentAlert('Contraseña insegura', 'La contraseña debe ser de al menos 6 caracteres');
          reject('Contraseña débil. 6 caracteres mínimo');
        } else {
          reject('Error al registrar. Intenta de nuevo');
        }
      }
